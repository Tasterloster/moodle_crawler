...
${bodyHtml}
  ...
  async function run(config) {
    const { course, options } = config;
    if (!course || !course.url) throw new Error("Kein Kurs angegeben.");
    ...
    let rootUrl = config.rootUrl || course.url;
    if (!rootDoc) {
    ...
    progress("Archiv wird geschrieben …");
