...
(function () {
  ...
  const { t, formatDate } = window.MoodleCrawlerI18n;
  ...
  function sanitize(name, fallback) {
    if (!fallback) fallback = t("fallback.unnamed");
  ...
  async function request(url, options = {}) {
    ...
    throw lastError || new Error(t("err.request", { url }));
  }
  ...
  function detectCourse(doc, pageUrl) {
    ...
    if (!name) name = t("fallback.courseNamed", { id });
  ...
  function parseCoursePage(doc, baseUrl) {
    ...
    const collect = (node) => {
      ...
      if (!name) name = t("fallback.sectionNumbered", { n: numberAttr || index });
    ...
    if (sectionNodes.size === 0) {
      ...
      if (activities.length) {
        sections.push({
          key: "flat", index: 0, name: t("fallback.contentFlat"),
          summaryHtml: "", summaryEl: null, activities,
        });
      }
    } else {
  ...
  function parseActivity(node, baseUrl) {
    ...
    if (!href) return null;
    if (!name) name = cleanText(anchor) || t("fallback.activity", { id: cmid || "" });
    ...
    const runners = new Array(Math.min(limit, queue.length || 1)).fill(0).map(async () => {
      while (queue.length) {
        ...
        try {
          ...
          log(t("log.skipped", { reason: err && err.message ? err.message : err }), "warn");
        }
      }
    });
  ...
  async function downloadBinary(ctx, url) {
    ...
    if (limit && declared > limit) {
      state.stats.skipped++;
      log(t("log.tooLarge", { mb: (declared / 1048576).toFixed(1), what: url }), "warn");
      return null;
    }
    ...
    if (limit && bytes.length > limit) {
      state.stats.skipped++;
      log(t("log.tooLarge", { mb: (bytes.length / 1048576).toFixed(1), what: url }), "warn");
      return null;
    }
  ...
  async function saveFile(ctx, url, dir, hintName) {
    ...
    let name = result.name || hintName || t("fallback.file");
    if (hintName && !result.name) name = hintName;
    name = ensureExtension(sanitize(name, t("fallback.file")), result.type);
  ...
  async function saveAsset(ctx, url) {
    ...
    try {
      ...
      const fallbackImage = t("fallback.image");
      let name = ensureExtension(sanitize(result.name || fallbackImage, fallbackImage), result.type);
      const path = await ctx.archive.add(`${ctx.root}/${t("dir.images")}/${name}`, result.bytes);
      ...
      if (err && err.message === "__cancelled__") throw err;
      ctx.seenAssets.set(key, null);
      log(t("log.imageFailed", { url: key }), "warn");
      return null;
    }
  }
  ...
  function buildLinks(ctx) {
    ...
    render(t("heading.linksExternal"), ctx.links.filter((l) => l.external));
    render(t("heading.linksInternal"), ctx.links.filter((l) => !l.external));
    return lines.join("\n");
  }
  ...
  function wrapMarkdown(ctx, { title, type, sourceUrl, subtitle }, body) {
    const front = [
      "---",
      `${t("fm.title")}: ${mdYaml(title)}`,
      `${t("fm.course")}: ${mdYaml(ctx.course.name)}`,
    ];
    if (type) front.push(`${t("fm.type")}: ${mdYaml(type)}`);
    if (subtitle) front.push(`${t("fm.section")}: ${mdYaml(subtitle)}`);
    if (sourceUrl) front.push(`${t("fm.source")}: ${mdYaml(sourceUrl)}`);
    front.push(`${t("fm.saved")}: ${new Date().toISOString().slice(0, 10)}`);
...
${bodyHtml}
  ...
  async function saveContent(ctx, { dir, baseName, title, element, baseUrl, sourceUrl, subtitle, linkSource, type }) {
    ...
    const stem = await ctx.archive.reserveStem(
      `${dir}/${prefixed(ctx, dir, sanitize(baseName, t("fallback.content")))}`,
      extensions
    );
    ...
    clone.querySelectorAll("a[href]").forEach((a) => {
      ...
      if (/^https?:/i.test(href) || href.startsWith("..") || href.startsWith(t("dir.images") + "/")) return;
    ...
    for (const ext of extensions) {
      ...
      if (ext === ".html") {
        ...
        if (sourceUrl) {
          metaParts.push(`${t("meta.source")}: <a href="${escapeHtml(sourceUrl)}">${escapeHtml(sourceUrl)}</a>`);
        }
        metaParts.push(t("meta.savedAt", { date: formatDate(new Date()) }));
  ...
  async function handleResource(ctx, act, dir) {
    ...
    if (!/text\/html/i.test(type)) {
      ...
      if (limit && bytes.length > limit) {
        state.stats.skipped++;
        log(t("log.tooLarge", { mb: (bytes.length / 1048576).toFixed(1), what: act.name }), "warn");
        return { saved: [] };
      }
    ...
    if (!saved.length && ctx.options.texts) {
      const page = await saveContent(ctx, {
        dir, baseName: act.name, title: act.name, type: t("type.resource"),
  ...
  function submissionDir(ctx, dir, name) {
    const folder = t("suffix.ownSubmission", { name: sanitize(name, t("fallback.assign")) });
    return `${dir}/${prefixed(ctx, dir, folder)}`;
  }
  ...
  async function handleAssign(ctx, act, dir) {
    ...
    if (ctx.options.files) {
      ...
      if (ctx.options.submissions && own.length) {
        ...
        log(t("log.ownFiles", { n: own.length, name: act.name }));
      }
    }
    ...
    if (ctx.options.texts) {
      ...
      const page = await saveContent(ctx, {
        dir, baseName: act.name, title: act.name, type: t("type.assign"),
      ...
      if (ctx.options.submissions && intro && main) {
        ...
        if (rest.querySelector("table, .submissionstatustable, [data-region='submission-status']")) {
          const status = await saveContent(ctx, {
            dir, baseName: t("suffix.ownSubmission", { name: act.name }),
            title: t("label.ownSubmissionTitle", { name: act.name }),
            type: t("type.submission"), element: rest, baseUrl: finalUrl, sourceUrl: act.url,
            linkSource: t("linksrc.submission", { name: act.name }),
          });
  ...
  async function handleFolder(ctx, act, dir) {
    const { doc, finalUrl } = await fetchDocument(act.url);
    if (!doc) return { saved: [] };
    const target = `${dir}/${prefixed(ctx, dir, sanitize(act.name, t("fallback.folder")))}`;
    const urls = new Set();
    doc
  ...
  async function handlePage(ctx, act, dir) {
    ...
    const region =
      ...
      contentRegion(doc);
    const page = await saveContent(ctx, {
      dir, baseName: act.name, title: act.name, type: t("type.page"),
  ...
  async function handleBook(ctx, act, dir) {
    const { doc, finalUrl } = await fetchDocument(act.url);
    if (!doc) return { saved: [] };
    const target = `${dir}/${prefixed(ctx, dir, sanitize(act.name, t("fallback.book")))}`;
    ...
    for (const chapter of chapters) {
      ...
      const page = await saveContent(ctx, {
        dir: target,
        baseName: `${pad(n)} ${chapter.title || t("fallback.chapter", { n })}`,
        title: chapter.title || act.name,
        element: region, baseUrl: chapter.url, sourceUrl: chapter.url,
        subtitle: act.name, type: t("type.chapter"),
      });
      if (page) {
        saved.push(...page.paths);
        texts.push(`### ${chapter.title || t("fallback.chapter", { n })}\n\n${page.text}`);
      }
      n++;
    }
  ...
  async function handleForum(ctx, act, dir) {
    const { doc, finalUrl } = await fetchDocument(act.url);
    if (!doc) return { saved: [] };
    const target = `${dir}/${prefixed(ctx, dir, sanitize(act.name, t("fallback.forum")))}`;
    ...
    doc.querySelectorAll('a[href*="/mod/forum/discuss.php"]').forEach((a) => {
      ...
      seen.add(clean);
      discussions.push({ url: clean, title: cleanText(a) || t("fallback.discussion") });
    });
    ...
    for (const discussion of discussions.slice(0, 300)) {
      ...
      const page = await saveContent(ctx, {
        ...
        subtitle: act.name, type: t("type.post"),
      });
      if (page) {
        saved.push(...page.paths);
  ...
  async function handleUrl(ctx, act, dir) {
    ...
    if (!ctx.linkSeen.has(key)) {
      ...
      ctx.links.push({
        text: act.name,
        url: target,
        source: t("linksrc.urlActivities"),
  ...
  async function handleGeneric(ctx, act, dir) {
    ...
    if (ctx.options.texts) {
      ...
      const page = await saveContent(ctx, {
        ...
        element: region, baseUrl: finalUrl, sourceUrl: act.url,
        subtitle: typeLabel(act.type), type: typeLabel(act.type),
      });
      if (page) {
        saved.push(...page.paths);
        text = page.text;
  ...
  /** Übersetzte Bezeichnung eines Aktivitätstyps, sonst der rohe Typ. */
  ...
  function buildReadme(ctx, model) {
    const lines = [
      `# ${ctx.course.name}`,
      "",
      `* ${t("readme.source")}: ${ctx.course.url}`,
      `* ${t("readme.savedOn")}: ${formatDate(new Date())}`,
      `* ${t("readme.sections")}: ${model.length}`,
      `* ${t("stats.short", { files: state.stats.files, pages: state.stats.pages, images: state.stats.images })}`,
      `* ${t("readme.totalSize")}: ${(state.stats.bytes / 1048576).toFixed(1)} MB`,
      "",
      `## ${t("heading.contents")}`,
      "",
    ];
  ...
  function buildIndexMarkdown(ctx, model) {
    const indexPath = `${ctx.root}/${t("file.index")}`;
    const lines = [
      "---",
      `${t("fm.title")}: ${mdYaml(ctx.course.name)}`,
      `${t("fm.source")}: ${mdYaml(ctx.course.url)}`,
      `${t("fm.saved")}: ${new Date().toISOString().slice(0, 10)}`,
      "tags:", "  - moodle", "---", "",
      `# ${ctx.course.name}`, "",
      t("stats.full", {
        files: state.stats.files, pages: state.stats.pages,
        images: state.stats.images, mb: (state.stats.bytes / 1048576).toFixed(1),
      }),
      "",
    ];
    ...
    for (const entry of model) {
      ...
      for (const item of entry.items) {
        ...
        if (item.url) line += `  \n  [${t("label.openInMoodle")}](${mdUrl(item.url)})`;
        lines.push(line);
      }
      lines.push("");
    }
  ...
  function buildIndexHtml(ctx, model) {
    const parts = model.map((entry) => {
      const items = entry.items
        .map((item) => {
          const label = escapeHtml(typeLabel(item.type));
          const links = item.files
            .map((f) => `<a href="${escapeHtml(relPath(ctx.root + "/index.html", f))}">${escapeHtml(f.split("/").pop())}</a>`)
            .join(" · ");
          const online = item.url
            ? `<a class="src" href="${escapeHtml(item.url)}">${escapeHtml(t("label.original"))}</a>`
            : "";
          return `<li><span class="badge">${label}</span> <strong>${escapeHtml(item.name)}</strong>
            <div class="files">${links || `<em>${escapeHtml(t("label.noLocalFile"))}</em>`} ${online}</div></li>`;
        })
        .join("\n");
      return `<section><h2>${escapeHtml(entry.section.name)}</h2>
        ${entry.summaryText ? `<p class="summary">${escapeHtml(entry.summaryText).replace(/\n/g, "<br>")}</p>` : ""}
        <ul>${items || `<li><em>${escapeHtml(t("label.noContentHtml"))}</em></li>`}</ul></section>`;
    });

    const meta = `${escapeHtml(t("meta.source"))}: ` +
      `<a href="${escapeHtml(ctx.course.url)}">${escapeHtml(ctx.course.url)}</a> · ` +
      escapeHtml(t("stats.short", {
        files: state.stats.files, pages: state.stats.pages, images: state.stats.images,
      })) + " · " +
      escapeHtml(t("meta.savedAt", { date: formatDate(new Date()) }));
  ...
  async function processActivity(ctx, act, dir, number) {
    ...
    if (act.type === "label") {
      if (act.descEl) collectLinks(ctx, act.descEl, t("linksrc.label", { name: act.name }), ctx.course.url);
      if (ctx.options.texts && act.descEl) {
        const page = await saveContent(ctx, {
          dir, baseName: act.name, title: act.name,
          element: act.descEl, baseUrl: ctx.course.url, subtitle: t("sub.labelInCourse"),
          type: t("type.label"),
        });
        if (page) {
    ...
    if (act.descEl) {
      collectLinks(ctx, act.descEl, t("linksrc.description", { name: act.name }), ctx.course.url);
      ...
      if (hasMedia && ctx.options.texts && (ctx.options.images || ctx.options.files)) {
        const page = await saveContent(ctx, {
          dir, baseName: t("suffix.description", { name: act.name }),
          title: t("suffix.description", { name: act.name }),
          element: act.descEl, baseUrl: ctx.course.url, subtitle: t("sub.descInCourse"),
  ...
  async function fetchEnrolledCourses(origin, sesskey) {
    ...
    for (let offset = 0, page = 0; page < 40; page++, offset += limit) {
      ...
      list.forEach((c) => {
        courses.push({
          id: String(c.id),
          name: c.fullname || c.shortname || t("fallback.courseNumbered", { id: c.id }),
  ...
  async function discoverCourses(baseUrl) {
    ...
    if (!doc) return { ok: false, error: t("err.noOverview") };
    ...
    return { ok: false, error: t("err.noCourses") };
  }
  ...
  async function crawlCourse(target, course, options, providedDoc, providedUrl) {
    ...
    if (!rootDoc) {
      progress(t("progress.loadingCourse"));
      const loaded = await fetchDocument(course.url);
      if (!loaded.doc) throw new Error(t("err.coursePage"));
      rootDoc = loaded.doc;
    ...
    log(t("log.course", { name: course.name }));
    progress(t("progress.structure"));
    ...
    log(t("log.found", { sections: sections.length, items: state.total }));
    progress(t("progress.content"));
    ...
    for (const section of sections) {
      ...
      if (section.summaryEl) {
        collectLinks(ctx, section.summaryEl, t("linksrc.section", { name: section.name }), course.url);
      }
      if (section.summaryEl && ctx.options.texts) {
        ...
        if (summaryText) {
          ...
          ctx.sectionDir = dir;
          const page = await saveContent(ctx, {
            dir, baseName: t("name.sectionText"), title: section.name,
            element: section.summaryEl, baseUrl: course.url, subtitle: t("sub.sectionDesc"),
            type: t("type.sectionDesc"),
          });
      ...
      for (const act of section.activities) {
        ...
        try {
          ...
          if (err && err.message === "__cancelled__") throw err;
          log(t("log.itemFailed", { name: act.name, message: err && err.message ? err.message : err }), "error");
    ...
    progress(t("progress.overview"));

    // Gesammelte Texte als eine durchsuchbare Datei.
    if (ctx.options.texts) {
      const textParts = [
        "---",
        `${t("fm.title")}: ${mdYaml(t("heading.allTexts", { course: course.name }))}`,
        `${t("fm.course")}: ${mdYaml(course.name)}`,
        `${t("fm.source")}: ${mdYaml(course.url)}`,
        `${t("fm.saved")}: ${new Date().toISOString().slice(0, 10)}`,
        "tags:", "  - moodle", "---", "",
        `# ${t("heading.allTexts", { course: course.name })}`, "",
      ];
      for (const entry of model) {
        ...
        for (const item of entry.items) {
          ...
          textParts.push(`### ${item.name} (${typeLabel(item.type)})`, "", item.text, "");
        }
      }
      await ctx.archive.add(`${ctx.root}/${t("file.texts")}`, textParts.join("\n"));
    }

    if (ctx.links.length) {
      await ctx.archive.add(`${ctx.root}/${t("file.links")}`, buildLinks(ctx));
    }

    await ctx.archive.add(`${ctx.root}/README.md`, buildReadme(ctx, model));
    const extensions = formatsFor(ctx.options);
    if (extensions.includes(".md")) {
      await ctx.archive.add(`${ctx.root}/${t("file.index")}`, buildIndexMarkdown(ctx, model));
    }
  ...
  function buildCourseIndex(overview) {
    const lines = [
      "---",
      `${t("fm.title")}: ${mdYaml(t("heading.courses"))}`,
      `${t("fm.saved")}: ${new Date().toISOString().slice(0, 10)}`,
      "tags:", "  - moodle", "---", "",
      `# ${t("heading.savedCourses")}`, "",
    ];
    overview.forEach((o) => {
      const items = o.model.reduce((n, e) => n + e.items.length, 0);
      lines.push(`- [${o.course.name}](${mdUrl(o.root + "/README.md")}) — ${t("label.items", { n: items })}`);
    });
    return lines.join("\n");
  }
  ...
  async function run(config) {
    ...
    if (!courses.length) throw new Error(t("err.noCourse"));
    courses.forEach((c) => {
      if (!c || !c.url) throw new Error(t("err.courseWithoutUrl"));
    });

    window.MoodleCrawlerI18n.setLanguage(options.language);
    hooks = {
    ...
    try {
      if (combined) {
        ...
        await target.archive.add(t("file.allCourses"), buildCourseIndex(overview));
        const blob = target.zip.close();
        const filename = t("zip.allCourses", { date: stamp });
        progress(t("progress.saving"));
        ...
        for (const course of courses) {
          ...
          await hooks.download(blob, filename);
          results.push({ filename, size: blob.size });
          log(t("log.finished", { file: filename, mb: (blob.size / 1048576).toFixed(1) }));
        }
      }
    } finally {
      state.running = false;
